...
class GaussianScene(nn.Module):
    def __init__(self, colmap_path: str, gaussians: Gaussians) -> None:
        super().__init__()
    ...
    def render_points_image(self, image_idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        return self.images[image_idx].project_point_to_camera_perspective_projection(self.gaussians.points, self.gaussians.colors)

    def get_2d_covariance(self, image_idx: int, points: torch.Tensor, covariance_3d: torch.Tensor) -> torch.Tensor:
        """
    ...
    def preprocess(self, image_idx: int) -> None:
        ...
        in_view = in_view_frustum(points=self.gaussians.points, view_matrix=self.images[image_idx].world2view)
        ...
        points_view = (points_homogeneous @ self.images[image_idx].world2view.to(points_homogeneous.device))[:, :3]

        points_ndc = points_homogeneous @ self.images[image_idx].full_proj_transform.to(points_homogeneous.device)
        ...
        covariance_2d = self.get_2d_covariance(image_idx=image_idx, points=points, covariance_3d=covariance_3d)
    ...
    def render_pixel(
        ...
        for point_idx in range(points_in_tile_mean.shape[0]):
            ...
            weight = compute_gaussian_weight(pixel_coord=pixel_coords, point_mean=point, inverse_covariance=inverse_covariance[point_idx])
    ...
    def render_tile(
        self, x_min: int, y_min: int, points_in_tile_mean: torch.Tensor, colors: torch.Tensor, opacities: torch.Tensor, inverse_covariance: torch.Tensor, tile_size: int = 16
    ) -> torch.Tensor:
        ...
        for pixel_x in range(x_min, x_min + tile_size):
            for pixel_y in range(y_min, y_min + tile_size):
                tile[pixel_x % tile_size, pixel_y % tile_size] = self.render_pixel(
                    pixel_coords=torch.Tensor([pixel_x, pixel_y]).view(1, 2).to(points_in_tile_mean.device),
    ...
    def render_image(self, image_idx: int, tile_size: int = 16) -> torch.Tensor:
        ...
        for x_min in tqdm(range(0, width - tile_size, tile_size)):
            ...
            for y_min in range(0, height - tile_size, tile_size):
                ...
                inverse_covariance_in_tile = preprocessed_scene.inverse_covariance_2d[points_in_tile]
                image[x_min : x_min + tile_size, y_min : y_min + tile_size] = self.render_tile(
                    x_min=x_min,
                    y_min=y_min,
                    points_in_tile_mean=points_in_tile_mean,
    ...
    def compile_cuda_ext(self,) -> torch.jit.ScriptModule:
