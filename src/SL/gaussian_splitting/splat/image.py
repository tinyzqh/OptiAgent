...
from splat.utils import build_rotation, focal2fov, get_extrinsic_matrix, get_intrinsic_matrix, getProjectionMatrix, getWorld2View, in_view_frustum, ndc2Pix
...
class GaussianImage(torch.nn.Module):
    def __init__(self, camera: Camera, image: Image) -> None:
        ...
        self.intrinsic_matrix = get_intrinsic_matrix(f_x=self.f_x, f_y=self.f_y, c_x=self.c_x, c_y=self.c_y).to(self.device)
        ...
        self.full_proj_transform = (self.world2view.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0).to(self.device)
    ...
    def project_point_to_camera_perspective_projection(self, points: torch.Tensor, colors: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # first check if the points will even be in view
        in_frustum_truth = in_view_frustum(points=points, view_matrix=self.world2view)
